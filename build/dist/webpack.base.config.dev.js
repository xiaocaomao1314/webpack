...
var webpack = require('webpack'); // 内存和dist生产index.html文件 并且自动导入打包好的bundle.js文件


var htmlPlugin = require('html-webpack-plugin'); // 打包后 dist目录删除后 再生成新的dist 保证打包后的目录的干净
// 推荐mode:production使用
//const { CleanWebpackPlugin } = require('clean-webpack-plugin');
// 该插件将CSS提取到单独的文件中。 它为每个包含CSS的JS文件创建一个CSS文件。 它支持CSS和SourceMap的按需加载。


var MiniCssExtractPlugin = require('mini-css-extract-plugin'); //查看打包好的文件分布情况
...
module.exports = {
  ...
  entry: {
    ...
    other: ["./src/other.js", "@babel/polyfill"] // a1: ["./src/a1.js", "@babel/polyfill"],
  ...
  // 插件
  plugins: [//内存中以某某html文件为模块 生产新的index.html文件 并且自动导入内存中的bundle.js文件
  // new htmlPlugin({
  ...
  new htmlPlugin({
    filename: 'index.html',
    //在导出到目录下生产的html5文件  默认文件名为index.html
    template: path.join(__dirname, '../index.html'),
    //以这个以某某html5文件为模板生产对应的html5文件
    inject: 'body',
    minify: {
  ...
  // }),
  new webpack.IgnorePlugin(/^\.\/locale/, /moment$/), // new MiniCssExtractPlugin({
  ...
  optimization: {
    ...
    splitChunks: {
      ...
      chunks: 'all',
      //async只针对异步加载模块进行拆分 可选值有all | initial  all针对静态页面进行分离
      minSize: 0,
      // 模块最少大于30KB才进行拆分
      maxSize: 0,
      // 如果超出maxSize 会进一步进行拆分  最好设置为0
      minChunks: 1,
      //模块最少引用一次就进行拆分  模块引入最少一次才进行拆分
      // axAsyncRequests: 30, //异步加载时发送的请求数量 不超过30  超过30不拆分
      maxInitialRequests: 30,
      //页面初始化时间发送的请求数量最大不能超过30 超过30不拆分 
      automaticNameDelimiter: '~',
      //默认的连接符
      name: true,
      //拆分的chunk名自动生成
      cacheGroups: {
        // 缓存组配置 上面配置读取完成后进行拆分 如果需要多个模块拆分到一个文件  就需要缓存 所以为缓存组
        //自定义缓存组名
        // vendors: {
        //     name: 'vendors',
  ...
  module: {
    ...
    rules: [{
      ...
      use: {
        ...
        options: {
          limit: 5 * 1024,
          //基本是限制5kb  5kb以上的图片用web路径 以下用base64
          outputPath: 'images',
          //设置图片存放的目录位置 用了webpack-dev-server 图片目录在内存中生存 并且存放图片
          name: "[name]-[hash:4].[ext]",
          //设置图片的名称 默认为hash格式的名字 [name]图片原名称 hash解析后的 ext代表图片格式jpg,png等
          esModule: false // [hash:4] 代表hash只有前面四个展示 最好越长越好
